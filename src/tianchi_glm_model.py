...
class lgm2d(object): 
    ...
    def _initialize_weights(self):
        weights = {
            'f1': tf.Variable(tf.random_normal([self.n_steps * self.n_height * self.n_width * self.n_length, self.n_output])),
    ...
    def saveModel(self, i, j):
        if not os.path.exists('../data/glm_model/'):
            os.mkdir('../data/glm_model/')
        saver = tf.train.Saver()
        saver.save(self.sess, "../data/glm_model/glm_model_{}.ckpt".format(i))
